...
content_dirs = [os.path.join("images/content", i) for i in os.listdir("images/content")]
style_dirs = [os.path.join("images/style", i) for i in os.listdir("images/style")]

decoder_t7 = 'models/decoder.t7'
vgg_t7 = 'models/vgg_normalised.t7'

for content, style in itertools.product(content_dirs, style_dirs):
    alphas = [0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.0]
    for alpha in alphas:
        stylized, content_image, style_image = stylize(content, style, alpha, vgg_t7, decoder_t7)
        cv2.imwrite(os.path.join("outputs", "{}-{}-{}-v2.jpg".format(content, style, alpha)), stylized)
